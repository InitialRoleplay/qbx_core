local ConvertText = require 'bridge.qb.shared.convert-function'
local Shared = require 'bridge.qb.shared.main'
...
local function convert(menu)
    ...
    for _,button in pairs(menu) do
        ...
        if Shared.Items[button.icon] then icon = ("nui://%s/web/images/%s"):format('ox_inventory', Shared.Items[tostring(button.icon)].image) else icon = button.icon or nil end
