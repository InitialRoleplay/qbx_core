local createPhoneExport = require 'bridge.phone.shared.export-function'
...
---@return string
local function RandomNumber(length) -- luacheck: ignore
...
local function pNotifyGroup(group, header, msg, icon, colour, length)
    ...
    for _, v in pairs(EmploymentGroup[group].members) do
        lib.notify(v.Player, {
            title = header or "NO HEADER",
            description = msg or "NO MSG",
            type = 'info',
            duration = length or 7500
        })
    end
...
local function RemovePlayerFromGroup(src, groupID, disconnected)
    ...
    for k,v in pairs(g) do
        if v.Player == src then
            ...
            TriggerClientEvent('qb-phone:client:RefreshGroupsApp', -1, EmploymentGroup)
            if not disconnected then TriggerClientEvent("QBCore:Notify", src, "Tu as quitté le groupe", "primary") end
...
RegisterNetEvent("qb-phone:server:jobcenter_CreateJobGroup", function(data)
    local src = source
    local player = exports.qbx_core:GetPlayer(src)
    if Players[src] then TriggerClientEvent('QBCore:Notify', src, "Tu as déjà créer un groupe", "error") return end
...
lib.callback.register('qb-phone:server:GetGroupsApp', function(source)
...
RegisterNetEvent('qb-phone:server:jobcenter_JoinTheGroup', function(data)
    ...
    if Players[src] then return TriggerClientEvent('QBCore:Notify', src, "Tu fais déjà parti d'un groupe", "success") end
    ...
    Players[src] = true
    TriggerClientEvent('QBCore:Notify', src, "Tu as rejoint un groupe", "success")
    TriggerClientEvent('qb-phone:client:RefreshGroupsApp', -1, EmploymentGroup)
end)
...
lib.callback.register('qb-phone:server:getAllGroups', function(src)
    if Players[src] then
...
lib.callback.register('qb-phone:server:getAllGroups', function(source, csn)
    local Names = {}
...
local function CreateGroup(src, name, password)
	...
	id = id,
        status = "WAITING",
        GName = name,
        GPass = password or RandomNumber(7),
        Users = 1,
        leader = src,
        members = {
            {name = GetPlayerCharName(src), CID = Player.PlayerData.citizenid, Player = src}
