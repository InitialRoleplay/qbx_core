...
Api.lib.checksum = Sha256CheckSum
Api.exp.checksum = Sha256CheckSum
...
local function RegisterRequest(path, fn, method, header)
	...
	if catch(
		...
		'The given HTTP request method \''..method..'\' isnt allowed for usage!! change the configuration to enable it. (file: \'api/.ini/config.lua\', var: \'config.methods\')'
	) then return end
...
Api.lib.route = RegisterRequest
Api.exp.route = RegisterRequest
...
Api.lib.request = PreformRequest

--- returns the api library
--- @return table api library
local function GetLibrary()
	return Api.lib
end
Api.exp.api = GetLibrary

-- create exports for the created methods above
for fn_name, fn in next, Api.exp do
	exports(fn_name, fn)
end
